...
import org.hf.experi.extentreports.ExtentReporter;
...
public class BasePage {
	...
	private WebElement findElement(By byLocator) {
		WebElement element = (new WebDriverWait(driver, getTimeout())).until(ExpectedConditions.presenceOfElementLocated(byLocator));
		return element;
	}
	 ...
	 */
      public boolean checkcondition(By byLocator){
		...
		try {
		iselementPresent = driver.findElement(byLocator).isDisplayed();
   ...
   /**
	...
	public void Swipe(String direction, int count) {
      ...
      public void launchiOSApp(String bundleID){
	     ...
	     try {
	    	iOSDriver = (IOSDriver<WebElement>) driver;
		    iOSDriver.launchApp();
